...
@Component({
  selector: 'app-root',
  imports: [RouterOutlet, HeadBar, SearchBar, Footer, Favorites],
...
export class App {
  ...
  addFavorite(auto: Auto) {
    const isFavorite = this.favoriteAutos.some(fav => fav.id === auto.id);
    if (isFavorite) {
      this.favoriteAutos = this.favoriteAutos.filter(fav => fav.id !== auto.id);
    } else {
      this.favoriteAutos.push(auto);
    }
    this.saveFavorites();
  }

  removeFavorite(autoId: number) {
    this.favoriteAutos = this.favoriteAutos.filter(fav => fav.id !== autoId);
    this.saveFavorites();
  }

  toggleFavoritesModal() {
    this.showFavoritesModal = !this.showFavoritesModal;
  }
  ...
  saveFavorites() {
    localStorage.setItem('favoriteAutos', JSON.stringify(this.favoriteAutos));
  }

  loadFavorites() {
    const stored = localStorage.getItem('favoriteAutos');
    if (stored) {
      this.favoriteAutos = JSON.parse(stored);
    }
  }
